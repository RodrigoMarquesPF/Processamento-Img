# -*- coding: utf-8 -*-
"""
Created on Tue Dec  5 15:45:59 2023
...
imagem = cv2.imread('imgSalePimenta2.png')
...
for i in range(1, imagem.shape[0]):
    for j in range(1, imagem.shape[1]):
        ...
        mode_value = statistics.mode(flattened_list)
        ##median_value = numpy.median(flattened_list)
        imagem3[i, j]=mode_value
        imagem4[i, j]=max_value
        
        
        
cv2.imshow("imagem_cinza", imagemCinza)
cv2.imshow("imagem_Moda", imagem3)
cv2.imshow("imagem_MAX_Ordem", imagem4)
